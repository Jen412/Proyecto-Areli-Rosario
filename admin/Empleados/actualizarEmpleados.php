<?php 
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        //Ejecutar mientras no tenga errores
        if (empty($errores)) {
            $db = conectarDB();
            $query = "CALL  modificarEmpleados('$id','$nombre', '$apeP', '$apeM','$ciudad', '$estado', '$calle', '$numCasa', '$colonia',
            '$telefono', '$edad', '$curp', '$especialidad','$sexo');";
            $resultado = mysqli_query($db, $query);
            
            if ($resultado) {
                //redireccionando al usuario
                header('Location: /admin/Empleados/empleados.php?resultado=2');
            }
        }
    }
?>
